...
<body >
	...
	<table border="1px">
		...
		<%
			...
			Page getpage = (Page)request.getAttribute("page");
			int pageSize=getpage.getPageSize(); 
			int currentNum=getpage.getCurrentNum();
	...
	<%
		...
		if(currentNum==1*pageSize){
	%>
			<a href="QueryStudentByPageServlet?currentNum=0">首页</a>
			<a href="QueryStudentByPageServlet?currentNum=<%=currentNum-pageSize%>">上一页</a>
	<% 
		}
		else if(currentNum==0){
	%>
			<a href="QueryStudentByPageServlet?currentNum=<%=currentNum+pageSize%>">下一页</a>
			<a href="QueryStudentByPageServlet?currentNum=<%=totalPage*pageSize%>">尾页</a>
	<%
		}else{
	%>
		<a href="QueryStudentByPageServlet?currentNum=0">首页</a>
